...
local function unlock_group_links(msg, data, target)
  ...
  if group_link_lock == 'no' then
    return 'ارسال لینک از قبل قفل نشده'
  else
...
local function lock_group_all(msg, data, target)
  ...
  if group_all_lock == 'yes' then
    return 'تمام لاک ها از قبل قفل بودند'
  else
    data[tostring(target)]['settings']['all'] = 'yes'
    save_data(_config.moderation.data, data)
    return 'تمام لاک ها در گروه قفل شدند'
  end
end
...
local function unlock_group_all(msg, data, target)
  ...
  if group_all_lock == 'no' then
    return 'لاک ها از قبل قفل نشده است'
  else
    data[tostring(target)]['settings']['all'] = 'no'
    save_data(_config.moderation.data, data)
    return 'تمام لاک ها در گروه مجاز شدند'
  end
end
...
local function lock_group_etehad(msg, data, target)
  ...
  if group_etehad_lock == 'yes' then
    ...
    save_data(_config.moderation.data, data)
    return 'سویچ اتحاد هم اکنون فعال شد'
  end
end
...
local function unlock_group_etehad(msg, data, target)
  ...
  if group_etehad_lock == 'no' then
    return 'سویچ اتحاد از قبل فعال نبود'
  else
    data[tostring(target)]['settings']['etehad'] = 'no'
    save_data(_config.moderation.data, data)
    return 'سویچ اتحاد هم اکنون غیر فعال شد'
  end
end
...
local function lock_group_family(msg, data, target)
  ...
  if group_family_lock == 'yes' then
    ...
    save_data(_config.moderation.data, data)
    return 'سویچ خانوادگی هم اکنون فعال شد'
  end
end
...
local function unlock_group_family(msg, data, target)
  ...
  if group_family_lock == 'no' then
    return 'سویچ خانوادگی از قبل فعال نبود'
  else
    data[tostring(target)]['settings']['family'] = 'no'
    save_data(_config.moderation.data, data)
    return 'سویچ خانوادگی هم اکنون غیر فعال شد'
  end
end

local function lock_group_normaly(msg, data, target)
  if not is_momod(msg) then
    return
  end
  local group_normaly_lock = data[tostring(target)]['settings']['normaly']
  if group_normaly_lock == 'yes' then
    return 'سویچ نرمال از قبل فعال بود'
  else
    data[tostring(target)]['settings']['etehad'] = 'yes'
    save_data(_config.moderation.data, data)
    return 'سویچ نرمال هم اکنون فعال شد'
  end
end

local function unlock_group_normaly(msg, data, target)
  if not is_momod(msg) then
    return
  end
  local group_normaly_lock = data[tostring(target)]['settings']['normaly']
  if group_normaly_lock == 'no' then
    return 'سویچ نرمال از قبل فعال نبود'
  else
    data[tostring(target)]['settings']['normaly'] = 'no'
    save_data(_config.moderation.data, data)
    return 'سویچ نرمال هم اکنون غیر فعال شد'
  end
end
...
local function lock_group_leave(msg, data, target)
  ...
  if group_leave_lock == 'yes' then
    return 'خروج از سوپر گروه از قبل قفل بود'
  else
    data[tostring(target)]['settings']['leave'] = 'yes'
    save_data(_config.moderation.data, data)
    return 'خروج از سوپر گروه قفل شد'
  end
end
...
local function unlock_group_leave(msg, data, target)
  ...
  if group_leave_lock == 'no' then
    return 'خروج از سوپر گروه از قبل قفل نبود'
  else
    data[tostring(target)]['settings']['leave'] = 'no'
    save_data(_config.moderation.data, data)
    return 'خروج از سوپر گروه مجاز شد'
  end
end
...
local function lock_group_operator(msg, data, target)
  ...
  if group_operator_lock == 'yes' then
    return 'اوپراتوراز قبل قفل بود'
  else
    data[tostring(target)]['settings']['operator'] = 'yes'
    save_data(_config.moderation.data, data)
    return 'اوپراتور قفل شد'
  end
end
...
local function unlock_group_operator(msg, data, target)
  ...
  if group_operator_lock == 'no' then
    return 'اوپراتور از قبل قفل نبود'
  else
    data[tostring(target)]['settings']['operator'] = 'no'
    save_data(_config.moderation.data, data)
    return 'اوپراتور مجاز شد'
  end
end
...
local function lock_group_reply(msg, data, target)
  ...
  if group_reply_lock == 'yes' then
    return 'ریپلای کردن پیام از قبل قفل بود'
  else
    data[tostring(target)]['settings']['reply'] = 'yes'
    save_data(_config.moderation.data, data)
    return 'ریپلای کردن پیام در سوپر گروه قفل شد'
  end
end
...
local function unlock_group_reply(msg, data, target)
  ...
  if group_reply_lock == 'no' then
    return 'ریپلای کردن پیام از قبل قفل نیود'
  else
    data[tostring(target)]['settings']['reply'] = 'no'
    save_data(_config.moderation.data, data)
    return 'ریپلای کردن پیام در سوپر گروه مجاز شد'
  end
end
...
local function unlock_group_username(msg, data, target)
  ...
  if group_username_lock == 'no' then
    return 'ارسال یوزرنیم از قبل قفل نبود'
  else
...
local function lock_group_media(msg, data, target)
  ...
  if group_media_lock == 'yes' then
    return 'رسانه از قبل قفل بود'
  else
    data[tostring(target)]['settings']['media'] = 'yes'
    save_data(_config.moderation.data, data)
    return 'رسانه در سوپر گروه قفل شد'
  end
end
...
local function unlock_group_media(msg, data, target)
  ...
  if group_media_lock == 'no' then
    return 'رسانه در سوپر گروه از قبل قفل نبود'
  else
    data[tostring(target)]['settings']['media'] = 'no'
    save_data(_config.moderation.data, data)
    return 'رسانه در سوپر گروه مجاز شد'
  end
end
...
local function lock_group_fosh(msg, data, target)
  ...
  if group_fosh_lock == 'yes' then
    return 'فحش دان از قبل قفل بود'
  else
    data[tostring(target)]['settings']['fosh'] = 'yes'
    save_data(_config.moderation.data, data)
    return 'فحش دادن در سوپر گروه قفل شد'
  end
end
...
local function unlock_group_fosh(msg, data, target)
  ...
  if group_fosh_lock == 'no' then
    return 'فحش دادن از قبل قفل نبود'
  else
    data[tostring(target)]['settings']['fosh'] = 'no'
    save_data(_config.moderation.data, data)
    return 'فحش دادن در سوپر گروه مجاز شد'
  end
end
...
local function lock_group_join(msg, data, target)
  ...
  if group_join_lock == 'yes' then
    return 'جوین با لینک  از قبل قفل بود'
  else
    data[tostring(target)]['settings']['join'] = 'yes'
    save_data(_config.moderation.data, data)
    return 'جوین با لینک در سوپر گروه قفل شد'
  end
end
...
local function unlock_group_join(msg, data, target)
  ...
  if group_join_lock == 'no' then
    return 'جوین با لینک از قبل قفل نشده بود'
  else
    data[tostring(target)]['settings']['join'] = 'no'
    save_data(_config.moderation.data, data)
    return 'جوین با لینک در سوپر گروه مجاز شد'
  end
end
...
local function lock_group_fwd(msg, data, target)
  ...
  if group_fwd_lock == 'yes' then
    return 'فروارد از قبل قفل بود'
  else
    data[tostring(target)]['settings']['fwd'] = 'yes'
    save_data(_config.moderation.data, data)
    return 'فروارد در سوپر گروه قفل شد'
  end
end
...
local function unlock_group_fwd(msg, data, target)
  ...
  if group_fwd_lock == 'no' then
    return 'فر.ارد از قبل قفل نیود'
  else
    data[tostring(target)]['settings']['fwd'] = 'no'
    save_data(_config.moderation.data, data)
    return 'فروارد در سوپر گروه مجاز شد'
  end
end
...
local function lock_group_english(msg, data, target)
  ...
  if group_english_lock == 'yes' then
    ...
    save_data(_config.moderation.data, data)
    return 'حرف زدن با زبان انگلیسی و فینگیلیش قفل شد'
  end
end
...
local function unlock_group_english(msg, data, target)
  ...
  if group_english_lock == 'no' then
    return 'زبان انگلیسی از قبل قفل نبود'
  else
    data[tostring(target)]['settings']['english'] = 'no'
    save_data(_config.moderation.data, data)
    return 'حرف زدن با زبان انگلیسی و فینگیلیش مجاز شد'
  end
end
...
local function unlock_group_emoji(msg, data, target)
  ...
  if group_emoji_lock == 'no' then
    return 'ارسال اموجی از قبل قفل نبود'
  else
...
local function lock_group_tag(msg, data, target)
  ...
  if group_tag_lock == 'yes' then
    return 'تگ از قبل قفل بود'
  else
    data[tostring(target)]['settings']['tag'] = 'yes'
    save_data(_config.moderation.data, data)
    return 'استفاده از تگ در سوپر گروه قفل شد'
  end
end
...
local function unlock_group_tag(msg, data, target)
  ...
  if group_tag_lock == 'no' then
    return 'تگ از قبل قفل نبود'
  else
    data[tostring(target)]['settings']['tag'] = 'no'
    save_data(_config.moderation.data, data)
    return 'استفاده از تگ در سوپر گروه مجاز شد'
  end
end
...
local function lock_group_spam(msg, data, target)
  ...
  if group_spam_lock == 'yes' then
    return 'اسپم زدن از قبل قفل بود'
  else
    data[tostring(target)]['settings']['lock_spam'] = 'yes'
    save_data(_config.moderation.data, data)
    return 'اسپم زدن در سوپر گروه قفل شد'
  end
end
...
local function unlock_group_spam(msg, data, target)
  ...
  if group_spam_lock == 'no' then
    return 'اسپم زدن از قبل قفل نبود'
  else
    data[tostring(target)]['settings']['lock_spam'] = 'no'
    save_data(_config.moderation.data, data)
    return 'اسپم زدن در سوپر گروه مجاز شد'
  end
end
...
local function lock_group_flood(msg, data, target)
  ...
  if group_flood_lock == 'yes' then
    return 'فلود از قبل قفل بود'
  else
    data[tostring(target)]['settings']['flood'] = 'yes'
    save_data(_config.moderation.data, data)
    return 'فلود در سوپر گروه قفل شد'
  end
end
...
local function unlock_group_flood(msg, data, target)
  ...
  if group_flood_lock == 'no' then
    return 'فلود از قبل قفل نبود'
  else
    data[tostring(target)]['settings']['flood'] = 'no'
    save_data(_config.moderation.data, data)
    return 'فلود در سوپر گروه مجاز شد'
  end
end
...
local function lock_group_arabic(msg, data, target)
  ...
  if group_arabic_lock == 'yes' then
    return 'زبان عربی و فارسی از قبل قفل بود'
  else
    data[tostring(target)]['settings']['lock_arabic'] = 'yes'
    save_data(_config.moderation.data, data)
    return 'استفاده از زبان فارسی و عربی در سوپر گروه قفل شد'
  end
end
...
local function unlock_group_arabic(msg, data, target)
  ...
  if group_arabic_lock == 'no' then
    return 'زبان عربی و فارسی از قبل قفل نبود'
  else
    data[tostring(target)]['settings']['lock_arabic'] = 'no'
    save_data(_config.moderation.data, data)
    return 'استفاده از زبان فارسی و عربی در سوپر گروه مجاز شد'
  end
end
...
local function lock_group_rtl(msg, data, target)
  ...
  if group_rtl_lock == 'yes' then
    return 'ار تی ال از قبل قفل بود'
  else
...
local function unlock_group_rtl(msg, data, target)
  ...
  if group_rtl_lock == 'no' then
    return 'ار تی ال از قبل قفل نبود'
  else
...
local function lock_group_tgservice(msg, data, target)
  ...
  if group_tgservice_lock == 'yes' then
    return 'نمایش ورود و خروج از قبل قفل بود'
  else
    data[tostring(target)]['settings']['lock_tgservice'] = 'yes'
    save_data(_config.moderation.data, data)
    return 'نمایش ورود و خروج کاربران از سوپر گروه قفل شد'
  end
end
...
local function unlock_group_tgservice(msg, data, target)
  ...
  if group_tgservice_lock == 'no' then
    return 'نمایش ورود و خروج از قبل قفل نبود'
  else
    data[tostring(target)]['settings']['lock_tgservice'] = 'no'
    save_data(_config.moderation.data, data)
    return 'نمایش ورود و خروج کاربران از سوپر گروه مجاز شد'
  end
end
...
local function unlock_group_sticker(msg, data, target)
  ...
  if group_sticker_lock == 'no' then
    return 'ارسال استیکر از قبل قفل نبود'
  else
...
local function unlock_group_bots(msg, data, target)
  ...
  if group_bots_lock == 'no' then
    return 'دعوت کردن ربات از قبل قفل نبود'
  else
...
local function lock_group_contacts(msg, data, target)
  ...
  if group_contacts_lock == 'yes' then
    return 'شیر اکانت کردن از قبل قفل بود'
  else
    data[tostring(target)]['settings']['lock_contacts'] = 'yes'
    save_data(_config.moderation.data, data)
    return 'شیر اکانت کردن  در سوپر گروه قفل شد'
  end
end
...
local function unlock_group_contacts(msg, data, target)
  ...
  if group_contacts_lock == 'no' then
    return 'شیر اکانت کردن از قبل قفل نبود'
  else
    data[tostring(target)]['settings']['lock_contacts'] = 'no'
    save_data(_config.moderation.data, data)
    return 'شیر اکانت کردن  در سوپر گروه مجاز شد'
  end
end
...
local function enable_strict_rules(msg, data, target)
  ...
  if group_strict_lock == 'yes' then
    return 'تنظیمات از قبل سخت گیر بود'
  else
    data[tostring(target)]['settings']['strict'] = 'yes'
    save_data(_config.moderation.data, data)
    return 'تنظیمات سخت گیرانه در سوپر گروه فعال شد'
  end
end
...
local function disable_strict_rules(msg, data, target)
  ...
  if group_strict_lock == 'no' then
    return 'تنظیمات از قبل سخت گیر نبود'
  else
    data[tostring(target)]['settings']['strict'] = 'no'
    save_data(_config.moderation.data, data)
    return 'تنظیمات سخت گیرانه در سوپر گروه غیر فعال شد'
  end
end
...
function show_supergroup_settingsmod(msg, target)
  ...
  local text = "\n#تنظیمات سوپر گروه@:\n\n🔐#قفل لینک : "..settings.lock_link.."\n🔐#قفل اکانت: "..settings.lock_contacts.."\n🔐#قفل فلود: "..settings.flood.."\n⛔️#مقدار حساسیت به اسپم: "..NUM_MSG_MAX.."\n🔐#قفل اسپم: "..settings.lock_spam.."\n🔐#قفل عربی/فارسی: "..settings.lock_arabic.."\n🔐#قفل دعوت کردن: "..settings.lock_member.."\n🔐#قفل ار تی ال: "..settings.lock_rtl.."\n🔐#قفل نمایش ورود /خروج: "..settings.lock_tgservice.."\n🔐#قفل استیکر: "..settings.lock_sticker.."\n🔐#قفل تگ: "..settings.tag.."\n🔐#قفل اموجی: "..settings.emoji.."\n🔐#قفل انگلیسی: "..settings.english.."\n🔐#قفل فروارد: "..settings.fwd.."\n🔐#قفل ریپلای: "..settings.reply.."\n🔐#قفل جوین با لینک: "..settings.join.."\n🔐#قفل یوزرنیم: "..settings.username.."\n🔐#قفل رسانه: "..settings.media.."\n🔐#قفل فحش: "..settings.fosh.."\n🔐#قفل برگشتن به گروه: "..settings.leave.."\n🔐#قفل ربات api: "..bots_protection.."\n🔐#قفل خدمات ارتباطی: "..settings.operator.."\n🔐#قفل تمام لاک ها: "..settings.all.."\n👥#عمومی بودن گروه: "..settings.public.."\n❌#تنظیمات سخت گیرانه: "..settings.strict.."\n🔱#سویچ نوع : اتحاد : "..settings.etehad.."\n☣#سویچ نوع : نرمال : "..settings.lock_rtl.."\n👪#سویچ نوع : خانوادگی : "..settings.lock_sticker.."\n\n✡#نوع گروه : "..gp_type.."\n🌀#کانال ربات:\n@KINGBOTTEAM\n__________\n❤️😐سودوهای های ربات :\n@mehdiisudo\n@mohammad20162015`"
  return text
end
...
local function run(msg, matches)
	...
	local data = load_data(_config.moderation.data)
		...
		lock_group_operator(msg, data, target),
      	...
      	return lock_group_normaly(msg, data, target), normaly
      end
		...
		unlock_group_operator(msg, data, target),
      	...
      	return unlock_group_normaly(msg, data, target), dnormaly
      end
			     if matches[2] == 'family' then
      	local dfamily ={
        unlock_group_links(msg, data, target),
		...
		unlock_group_flood(msg, data, target),
		unlock_group_rtl(msg, data, target),
		unulock_group_tgservice(msg, data, target),
		unlock_group_sticker(msg, data, target),
		unlock_group_contacts(msg, data, target),
		unlock_group_username(msg, data, target),
		unlock_group_fosh(msg, data, target),
		...
		if matches[1] == 'setflood' then
			...
			if tonumber(matches[2]) < 1 or tonumber(matches[2]) > 400 then
				return "Wrong number,range is [1_400]"
			end
		...
		if matches[1] == "muteuser" or matches[1] == "unsilent" and is_momod(msg) then
			local chat_id = msg.to.id
			local hash = "mute_user"..chat_id
			local user_id = ""
			if type(msg.reply_id) ~= "nil" then
			...
			elseif matches[1] == "muteuser" or matches[1] == "unsilent" and string.match(matches[2], '^%d+$') then
				local user_id = matches[2]
				if is_muted_user(chat_id, user_id) then
					unmute_user(chat_id, user_id)
		...
		if matches[1] == 'help' and not is_owner(msg) then
			text = "دوست عزیزوگرامی شما به این دستور دسترسی نداریید و فقط اونر ها/وادمینهای ربات/و سودوهای ان دسترسی درند :)"
